...
object OnDisplayModule {
    @Provides
    @Singleton
    fun provideOnDisplayDataSource(@ApplicationContext context: Context): OnDisplaySettingsDataSource {
        return OnDisplaySettingsDataSource(context)
    }

    @Provides
    @Singleton
    fun provideOnDisplayRepository(dataSource: OnDisplaySettingsDataSource): OnDisplayRepository {
